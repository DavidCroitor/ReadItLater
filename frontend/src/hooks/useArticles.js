...
export default function useArticles() {
  ...
  const { user, isAuthenticated, token } = useProfiles();
    ...
    const fetchArticles = async () => {
      ...
      try {
        ...
        if (isAuthenticated && user?.id) {
          ...
          userArticles = MOCK_ARTICLES.filter(article => article.userId === user.id);
        } else {
    ...
    const handleAddToCategory = (id) => {
    console.log(`Add article with id: ${id} to category`);
    };

    const handleRead = (id) => {
    console.log(`Marking article with id: ${id} as read`);
    ...
    console.log(`Marking article with id: ${id} as favorite`);
    setArticles(articles.map(article => 
    ...
    console.log(`Adding new article: ${JSON.stringify(newArticle)}`);
